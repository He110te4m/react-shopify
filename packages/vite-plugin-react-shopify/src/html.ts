...
import {
  CSS_EXTENSIONS_REGEX,
  KNOWN_CSS_EXTENSIONS,
  hotReloadScriptId,
  hotReloadScriptUrl,
} from "./constants";
import type { DevServerUrl, FrontendURLResult } from "./types";
...
export default function shopifyHTML(options: ResolvedOptions): Plugin {
  ...
  const viteTagSnippetPrefix = (config: ResolvedConfig): string =>
    viteTagDisclaimer +
    viteTagEntryPath(config.resolve.alias, options.entrypointsDir, viteTagSnippetName);
    ...
    configureServer({ config, middlewares, httpServer }) {
      ...
      httpServer?.once("listening", () => {
        ...
        if (isAddressInfo(address)) {
          ...
          setTimeout(() => {
            if (options.tunnel !== false && frontendUrl !== "") {
              tunnelUrl = frontendUrl;
              console.log(`${viteDevServerUrl} is tunneled to ${tunnelUrl}`);
            }
          }, 100);

          const viteTagSnippetContent =
            viteTagSnippetPrefix(config) +
            viteTagSnippetDev(
              frontendUrl !== "" ? frontendUrl : viteDevServerUrl,
        ...
        middlewares.use((req, res, next) => {
          if (req.url === "/index.html") {
            ...
            res.end(
              fs
                .readFileSync(
                  path.join(path.dirname(fileURLToPath(import.meta.url)), "dev-server-index.html"),
                )
                .toString(),
            );
          }
    ...
    closeBundle() {
      ...
      Object.keys(manifest).forEach((src) => {
        ...
        if (isEntry === true) {
          ...
          assetTags.push(
            viteEntryTag(entryPaths, tagsForEntry.join("\n  "), assetTags.length === 0),
          );
        }
 ...
 */
function generateFrontendURL(options: ResolvedOptions): FrontendURLResult {
