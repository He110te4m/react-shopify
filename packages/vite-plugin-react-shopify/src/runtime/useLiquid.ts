...
export function useLiquidState<T = string>(path: string, opts?: UseLiquidOptions): [T, Setter<T>];
export function useLiquidState<T = string>(
  path: string | ShopifyReference<T, any>,
  opts?: UseLiquidOptions,
): [T, Setter<T>] {
  const expression = isShopifyReference(path) ? compileShopifyReference(path) : path;
  ...
  const initial = useMemo(() => {
    if (ctx.phase === "ssg") {
      ...
      if (htmlExpressions?.has(expression)) {
        throw new Error(
          `${expression} contains HTML. Render it through <LiquidHtml> instead of useLiquidState().`,
        );
      }
      ...
      return ctx.read(expression, isShopifyReference(path) ? path : undefined) as T;
    }
  ...
  }, [ctx.phase, expression, id, type, opts?.bridge, opts?.defaultValue]);
...
/** Read a typed Shopify reference without exposing Liquid paths to business code. */
export function useShopifyValue<T>(
  reference: ShopifyReference<T, any>,
  opts?: UseLiquidOptions,
): T {
  return useLiquidState(reference, opts)[0];
}
...
export function useLiquidExpression<T = string>(expression: string | ShopifyReference<T, any>): T {
  ...
  if (ctx.phase !== "ssg" || ctx.runtime !== "static") {
    throw new Error(
      "useLiquidExpression() is server-only and requires shopifyEntry.runtime = 'static'.",
    );
  }
  return ctx.read(compiled) as T;
}
