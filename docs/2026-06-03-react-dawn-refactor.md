...
- **v4(当前)**:API 评审后修正
  - `useImageTag` → `<ImageTag>` 组件(避免 hydration mismatch)
  - `useShopifyAttributes` 移除(`{...string}` 不合法 + 插件已处理)
  - `useBlockLoop` → `<BlockSlot>` 组件(SSR blocks 不可知 + 冲突 `content_for`)
  - `useBlockContext` / `useBlockRouter` 移除
  - bridge 不分层(用户反馈: 与 Section/Block 无关)
  - ID 生成内部化(`useUniqueId` hook,主题开发者不感知)
  - 新增 hydration 12 条检测规则(详见独立 API 文档 §6.5)
...
- 详细 API 设计见独立文档 `docs/2026-06-03-plugin-api-design.md`
