...
> - v4: API 设计评审后修正 — `useImageTag` 改为 `<ShopifyImage>`,`useShopifyAttributes` 移除,`useBlockLoop` 改为 `<BlockSlot>`,bridge 不分层
> - v5: 统一 ID 决策(自增计数器,无外部依赖)
>   - v6-v9: 评审反馈收敛 — `<ShopifyImage>` 定版(`image_url | image_tag` 完整管道 + `dangerouslySetInnerHTML`),`<ShopifyVideo>` 新增
...
| 项目 | 数量 / 规模 |
|---|---|
| 原生 Liquid section | 54 个,合计 ~16,000 行(v8 修订,实际 Dawn 15.4.1 数) |
| 原生 Liquid snippet | 38 个,合计 ~6,000 行(v8 修订) |
| 原生 Liquid template | 12 个 JSON(v8 修订) |
...
| `assets/` 下的 SVG | 87 个图标 + 3 个装饰(v8 修订) |
| `locales/` | 51 个语言文件 |
...
├── snippets/                  38 native
├── templates/                 12 JSON
├── assets/                    64 CSS + 32 JS + 87 SVG(v10 修订)  ← **永久保留**作 diff 对照(React 内化版本经 Vite 处理,但原文件不动)
...
| SVG icons (87 个,v8 修订) | 内化为 `.tsx` 组件 | **改为按需 import 的 `Icon` 组件**(见 §3.2),原始 svg 在 `frontend/icons/svg-source/` 与 `assets/*.svg` **两处都保留** |
| 字体文件 (woff2) | 保留在 assets | **保留在 `assets/`,theme.liquid 引用**(字体由 Shopify font picker 配置,Dawn 的逻辑不变) |
| `image_picker` 图片 | 走 `<ShopifyImage>` 组件 | 同上(组件 + 标记替换,见 API 文档 §2.1) |
...
│
├── icons/                         # 87 个 svg → 1 个 Icon 组件 + 资源数据
...
│   └── url.ts
│
├── sections/                      # 54 个 .tsx(v8 修订)
│   └── *.tsx
│
├── blocks/                        # ★ Theme Blocks,见 §5(40+ 个)
│   └── *.tsx
│
└── snippets/                      # 38 个 .tsx(v8 修订)
    └── *.tsx
```
...
### 4.1 Icon 组件(取代 87 个分散 svg,v8 修订)
...
- `registry.ts`:类型 `IconName = 'caret' | 'arrow' | ...`(87 个字面量类型,v8 修订)
...
| **Theme Block(静态)** | block 位置固定 + 需要从父级接收数据 | `frontend/blocks/Xxx.tsx` + 父级用 `<StaticBlock type="xxx" id="..." data={{...}} />` |
...
| `_slideshow-controls` | (静态块) | **Theme Block(静态私有)** | `frontend/blocks/_SlideshowControls.tsx`,slideshow 用 `<StaticBlock>` 注入位置 |
...
- 需要新增 `<StaticBlock>` 组件(用于静态 block 注入位置)
...
> React 组件代码用 `<BlockSlot />` 组件占位,由 Shopify 的 `{% content_for 'blocks' %}` 机制自动迭代 block,每个 Theme Block 是独立的 React entry。**单一职责**:`ImageBanner.tsx` 只关心布局/样式/容器,具体 block 内容由 `blocks/Heading.tsx` 等独立维护。详见 API 文档 §4.1。
...
步骤 4: 显示逻辑重写
  ...
  - Liquid 表达式 `{{ x | y }}` → 对应 hook / 组件(`<ShopifyImage>` / `useT` i18n / `useSectionPadding` / ...)
  - Liquid 标签 `{% if %}` → 三元或 clsx
  - Liquid 标签 `{% for %}` / `{% case %}` → **`<BlockSlot />` 组件**(由 Shopify 迭代 block,每个 Theme Block 独立 React entry)
  - Liquid 标签 `{% form %}` / `{% paginate %}` → **v10 修正**:**阶段 1-3 不实现**,form/paginate 类的 section 阶段 4+ 重写(详见 API 设计 §3.2 §3.3)
  - Liquid 标签 `{% style %}` → `useSectionPadding` / `useStyleBlock`
  - `{{ block.shopify_attributes }}` → **无需 hook**,插件已自动注入到 block wrapper
  - `section.index` → `useLiquidValue("section.index", "number")`(v10:`useSectionIndex()` 暂未实现)
...
| `image_picker` | 第 58 行 | **`<ShopifyImage>` 组件 + 标记替换**(不能返回字符串) |
| `placeholder_svg_tag` | 第 87 行 | `<ImagePlaceholder name="hero-apparel-1" />` 组件 |
| `block.shopify_attributes` | 全文 | **无需 hook**,插件已自动注入到 block wrapper |
| `block.type` 分发(heading/text/buttons) | 第 120-164 行 | **`<BlockSlot />` 组件**(由 Shopify 迭代 block,每个 Theme Block 独立 React entry) |
...
import { ShopifyImage } from "~/components/ShopifyImage";         // ★ v8 组件
import { ImagePlaceholder } from "~/components/ImagePlaceholder";
import { BlockSlot } from "~/components/BlockSlot";       // ★ 替代 v1 的 useBlockLoop
import { SectionPadding } from "~/components/SectionPadding";  // ★ 统一内联样式入口
import { AspectRatio } from "~/components/AspectRatio";        // ★ adapt 模式专享
...
export const shopifyMeta = {
  ...
  blocks: [
    { type: "@theme" },   // ★ 接受所有 Theme Block(heading/text/buttons/...)
    { type: "@app" },     // ★ 接受 App Block
  ],
  presets: [
    { name: "Image banner", blocks: [{ type: "heading" }, { type: "text" }, { type: "buttons" }] }
  ],
...
export default function ImageBanner() {
  // 1. 读所有 settings(★ v10 修正:useSectionSettings 返回 { value },需解构)
  // 当前 API 签名: function useSectionSettings(key: string): { value: string | undefined }
  // 详见 packages/vite-plugin-react-shopify/src/runtime/hooks.ts:234
  const { value: image } = useSectionSettings("image");
  const { value: image2 } = useSectionSettings("image_2");
  const { value: overlayOpacity } = useSectionSettings("image_overlay_opacity");
  const { value: imageHeight } = useSectionSettings("image_height");
  const { value: imageBehavior } = useSectionSettings("image_behavior");
  const { value: desktopContentPosition } = useSectionSettings("desktop_content_position");
  const { value: desktopContentAlignment } = useSectionSettings("desktop_content_alignment");
  const { value: showTextBox } = useSectionSettings("show_text_box");
  const { value: colorScheme } = useSectionSettings("color_scheme");
  const { value: stackImagesOnMobile } = useSectionSettings("stack_images_on_mobile");
  const { value: mobileContentAlignment } = useSectionSettings("mobile_content_alignment");
  const { value: showTextBelow } = useSectionSettings("show_text_below");

  // 2. 全局 setting + 派生 boolean
  const { value: animationsRaw } = useThemeSettings("animations_reveal_on_scroll");
  const animationsEnabled = parseLiquidBoolean(animationsRaw);

  // 3. v11 修正:从 Liquid 表达式获取 section.id
  //   当前 SSG wrapper 已注入 id="{{ section.id }}",但 React 组件内部也可能用
  //   Dawn 原版在 .banner 元素上使用了 id="Banner-{{ section.id }}",所以这里显式获取
  const { value: sectionId } = useLiquidValue("section.id");

  // ★ v11 修正:useImageBehavior 不再导入(v8 ShopifyImage 不需要 widths/sizes)
  // 移除此 import: 见下

  // 4. 派生 className(★ v10 修正:不用 useBannerClassName,显式用 clsx)
  const isAdaptHeight = imageHeight === "adapt";
  const bannerClass = clsx(
    ...
    isAdaptHeight && image && "banner--adapt",
    showTextBelow && "banner--mobile-bottom",
    showTextBox === false && "banner--desktop-transparent",
    animationsEnabled && "scroll-trigger animate--fade-in"
  );

  return (
    <div id={`Banner-${sectionId}`} className={bannerClass}>
      {/* ★ v11 修正:统一内联样式入口,职责分明
          - <SectionPadding> 处理 .section-{{id}}-padding 媒体查询 + opacity
          - <AspectRatio> 仅 adapt 模式时输出 aspect ratio 媒体查询 */}
      <SectionPadding
        id={sectionId}
        paddingTop={imageHeight === "small" ? 36 : undefined}  // 实际值来自 useImageBehavior
        paddingBottom={undefined}
        overlayOpacity={overlayOpacity}
      />
      {isAdaptHeight && image && (
        <AspectRatio id={sectionId} image={image} />
      )}

      {/* 图片 1 — ★ v10: ShopifyImage v8 签名,默认 loading="lazy" */}
      {image && (
      ...
      {/* 内容区 — ★ BlockSlot 替代 v1 的 useBlockLoop + BlockRouter */}
...
```tsx
// ★ v2: 删除了 v1 的 BlockRouter.tsx — 不再需要
// 每个 Theme Block 独立 entry,直接被 Shopify 渲染
```

```tsx
// frontend/blocks/Heading.tsx(Theme Block — 独立 React entry)
...
export default function Heading() {
  // ★ v11 修正:useBlockSettings 与 useSectionSettings 签名一致,返回 { value }
  const { value: heading } = useBlockSettings("heading");
  const { value: size } = useBlockSettings("heading_size");
  return (
    <h2 className={clsx("banner__heading", "inline-richtext", `banner__heading--${size}`)}>
      {heading}
    </h2>
  );
}
...
| 块渲染顺序 | for block in section.blocks | `<BlockSlot />` 由 Shopify 迭代 | ✓ |
| block.attrs | 注入到 `<h2>` 等元素 | **插件自动注入到 block wrapper** | ✓ |
...
| `pagination.liquid` | 86 | `component-pagination.css`, 1 SVG | `Pagination.tsx`(阶段 4+ 配合 usePaginate,阶段 1-3 由 Liquid 驱动) | - |
...
### 8.1 运行时 hooks(v3 修订,反映 API 文档评审结果)

| 新 hook | 签名 | 用途 | v3 调整 |
|---|---|---|---|
| `useRawLiquid` | `(code: string) => void` | **v7 降级为 `useLiquidBlock` 别名**;不产生 DOM 节点,仅推入全局 `__shopify_ssg_liquid_blocks` | v10 修正:不在 React 树中占位 |
| `useForm` | — | **v10 决策**:**阶段 1 不实现**,阶段 4+ 重写(详见 API 设计 §3.2) | v10 修正:不是 B 类 hook |
| `usePaginate` | — | **v10 决策**:**阶段 1 不实现**,分页完全由 Liquid 处理(详见 API 设计 §3.3) | v10 修正:不是 B 类 hook |
| `useImageUrl` | `(image, width) => string` | 输出 `image_url` filter | 不变 |
| `useAsset` | `(path, mode?) => string` | asset_url / inline_asset_content | 不变 |
| `useFontFace` | `(font) => string` | 输出 @font-face 块 | 不变 |
| `useBlockType` | `(blocks, type) => BlockType[]` | 过滤 | 不变 |
| `usePlaceholderSvg` | `(name) => string` | `placeholder_svg_tag` | 不变 |
| `useSectionPadding` | `(settingsKey?) => ReactNode` | 媒体查询 padding | 不变 |
| `useColorScheme` | `(id) => string` | className | 不变 |
| `useThemeSettings` | `(key: string) => string \| undefined` | 读 theme 级 setting | 新增 |
| `useImageBehavior` | `(behavior: string) => { widths, sizes, stackedSizes, halfWidth, fullWidth, fetchPriority }` | 封装 `{%- liquid -%}` 块 | 新增(纯 JS) |
| `useSectionAspect` | `(id: string, image, adapt: boolean) => ReactNode` | 返回 `<style>` JSX 节点 | 新增(纯 JS) |
| `useIconUrl` | `(name: IconName) => string` | SSR 输出 Liquid 表达式 | 新增 |
| `useLocale` | `() => string` | 当前 locale 标识 | 新增(配合 i18n) |
| `useT` | i18n 版 `t(key, vars?)` | 从 React i18n 表读取 | **替换** 原 useT(走 Liquid filter) |
| `useStaticBlock` | — | — | **v10 移除**:改为 `<StaticBlock>` 组件(下条) |
| **组件(非 hook)** | | | |
| **`<ShopifyImage>`** | `(props: ShopifyImageProps) => JSX` | 输出 `image_url | image_tag` 完整 Liquid 表达式 | **已实施** |
| **`<ShopifyVideo>`** | `(props: ShopifyVideoProps) => JSX` | 输出 `video_tag` 完整 Liquid 表达式 | **已实施** |
| **`<BlockSlot>`** | `() => JSX` | 渲染 `{% content_for 'blocks' %}`,由 Shopify 迭代 block | **新增**(替代 v2 的 useBlockLoop) |
| **`<StaticBlock>`** | `(props: { type, id, data? }) => JSX` | 渲染 `{% content_for "block", ... %}`,指定 block 位置 | **新增**(替代 v8 的 useStaticBlock) |
| **移除**(v3) | | | |
| ~~`useImageTag`~~ | — | 输出 HTML 片段作为字符串 | **移除**:改为 `<ShopifyImage>` 组件,字符串会插入到 React 树中导致水合 mismatch |
| ~~`useBlockLoop`~~ | — | React 迭代 blocks | **移除**:SSR 时 blocks 数据不可知;与 `{% content_for 'blocks' %}` 机制冲突;改为 `<BlockSlot>` 组件 |
| ~~`useShopifyAttributes`~~ | — | 输出 `{{ block.shopify_attributes }}` 字符串 | **移除**:`{...string}` 不是合法 JSX;插件已自动注入到 block wrapper |
| ~~`useBlockContext`~~ | — | 读 block 数据 | **移除**:每 block 独立 React entry,直接 `useBlockSettings` 即可 |
| ~~`useBlockRouter`~~ | — | block.type 查表 | **移除**:每 block 独立 .tsx 文件,Shopify 直接渲染 |

### 8.2 SSG 编译侧

| 增强点 | v1 | v2 | v3 |
|---|---|---|---|
| CSS Modules | 启用 | 同(优先在组件级启用) | **不实施**(阶段 1 禁用,见 API 文档 §6.2) |
| `inline_asset_content` 跟踪 | ✓ | 同 | 同 |
| `placeholder_svg_tag` 跟踪 | ✓ | 同 | 同 |
| `t:` 表达式跟踪 | ✓ | **删除**(改 i18n) | 删除(同 v2) |
| ~~`useBlockLoop` 嵌套~~ | ✓ | 同 | **删除**(无 useBlockLoop) |
| `@app` 块 | ✓ | 同 | 同 |
| `cleanOrphans` 选项 | 提议 | **不实施** | 同 |
| `<StaticBlock>` 编译 | 不支持 | 新增 | v10 新增 |
| i18n locale 数据生成 | 不支持 | **新增** | 同 |
| **`<BlockSlot>` 编译** | — | — | **新增**:渲染为 `{% content_for 'blocks' %}`(已有机制,无需新实现) |
| **`<ShopifyImage>` 编译** | — | — | **新增**:SSR 时渲染为标记,后处理替换为完整 Liquid 表达式 |
| **不再依赖 `assets/build/` 目录的清理** | 提议 | 改:`emptyOutDir: true` | 同 |

### 8.3 Vite 配置侧

| 增强点 | v1 | v2 | v3 |
|---|---|---|---|
| `react-dom/server` 不进客户端 chunk | ✓ | 同 | 同 |
| `react-dom/client` 单独 vendor | ✓ | 同 | 同 |
| 监听模式清孤儿 | 提议 | **删除** | 同 |
| i18n 配置 | 不支持 | **新增** | 同 |
...
按顺序提交,每个 PR 包含实现 + 单测 + 文档(v3 修订):

| 子阶段 | 新 API | PR 标题 |
|---|---|---|
| 1.1 | `useRawLiquid`(仅 useLiquidBlock 别名,无新功能) | `chore(runtime): useRawLiquid alias to useLiquidBlock` |
| 1.2 | `useImageUrl`, `useAsset`, `useFontFace`, `useIconUrl`, `usePlaceholderSvg` | `feat(runtime): image, asset, icon hooks` |
| 1.3 | `<ShopifyImage>` 组件(v8 设计:真实 `<img>` + 默认 `loading="lazy"`) | `feat(runtime): ShopifyImage component (v8 design)` |
| 1.4 | `<BlockSlot>` + `<StaticBlock>` 组件(替代 v1 的 useBlockLoop) | `feat(runtime): BlockSlot and StaticBlock components` |
| 1.5 | i18n LocaleProvider + `useT`(新版) + `useLocale` + 插件 locale 生成 | `feat(i18n): react i18n system` |
| 1.6 | `useSectionPadding`, `useColorScheme`, `useThemeSettings`, `useImageBehavior`, `useSectionAspect`, `useBlockType`, `useSectionIndex`(新增) | `feat(runtime): style and theme hooks` |
| 1.7 | CSS Modules(可选,阶段 1 禁用), `manualChunks` 拆分, `<AppBlock>` 组件 | `feat(ssg): css modules, app block` |
| 1.8 | `inline_asset_content` / `placeholder_svg_tag` filter 跟踪,删 `t:` 跟踪 | `feat(ssg): filter tracking updates` |

**v10 调整说明**:
- 1.1 仅 `useRawLiquid`(降级为 useLiquidBlock 别名,**不引入 useForm/usePaginate**)
- 1.3 改为 v8 ShopifyImage 设计(真实 `<img>` + 默认 `loading="lazy"`,不支持 srcSet/sizes/fetchPriority)
- 1.6 新增 `useSectionIndex()` hook(评审 v3 §4.2 指出范例使用但未定义)

**v11 阶段 1 完工标准**(v11 新增,评审 v4 §4.1 补充):

阶段 1 完成的**硬性指标**:
1. **`react-shopify-example` 集成**:在 `examples/react-shopify-example/frontend/sections/` 中新增 1 个**端到端**的 React section,使用全部 8 个阶段 1 子阶段新增的 hook/组件
2. **本地验证**:`shopify theme dev` 启动后,浏览器 Console **零** hydration error(无 #418/#422/#423)
3. **单测通过**:每个新 hook 配套 Vitest 单元测试,覆盖率 ≥ 80%
4. **hydration 规则通过**:hydration-rules 模块的 H1-H13 默认级别,phase 1 example 编译时 0 error
5. **Dawn 对照**:`git diff dawn-original -- assets/` 为空(assets 未被动)

**v11 测试策略**(v11 新增,评审 v4 §4.3 补充):

| 测试类型 | 工具 | 范围 |
|---|---|---|
| **单元测试** | Vitest | 纯 JS utility(`useImageBehavior`, `useSectionPadding`, `clsx` 等) |
| **快照测试** | Vitest + `renderToString` | React 组件 SSR 输出(对比 Dawn原版 `render` 的 HTML) |
| **hydration 测试** | `hydrateRoot` + jsdom | 模拟 React 树与 DOM 对齐,验证无 #418 报错 |
| **端到端测试** | Playwright + screenshot diff | Dawn 原版 vs React 版本渲染对比 |
| **schema 验证** | `shopify theme check` | `shopifyMeta` settings/blocks/presets 合法性 |

**每完成一个 section**:
- 写 Vitest 单元测试(纯 JS 逻辑)
- 写 snapshot 测试(SSR HTML)
- 启动 `shopify theme dev` 截图对比 Dawn
- 记录到 §13 验收表

**CI 必跑**:
- `pnpm typecheck`(TS 严格)
- `pnpm test`(Vitest)
- `pnpm build`(Vite build 成功)
- `pnpm hydration:check`(hydration-rules 报告 0 error)

**v1 → v3 阶段 1 调整说明**:
- 1.1-1.2 不变
- 1.3 改为 `<ShopifyImage>` 组件(v1 错把 useImageTag 归到简单值 hook,实际是组件)
- 1.4 改为 `<BlockSlot>` + `<StaticBlock>` 组件(v1 的 useBlockLoop / useShopifyAttributes / useBlockRouter 全部废弃)
- 1.5 提前(原 1.3),i18n 仍是关键但实现独立
- 1.6 整合所有剩余的 utility hook
- 1.7 CSS Modules 改为可选(默认禁用)
- 1.8 filter 跟踪更新(原 1.7)

### 阶段 2 — 基础组件 + 简单 section(多 PR,每 PR 一个组件族)

- 2.1 `frontend/styles/*.css`(tokens / layout / typography / forms / animation, 拆分自 base.css)
- 2.2 `frontend/icons/*` 系统: `Icon.tsx` + 87 svg 源 + `registry.ts` 自动生成
...
- 5.2 替换 `templates/*.json`(12 个,v8 修订)中的 section type 为 `react-*`
...
| 10 | 备份原Dawn资源? | ✅ `git tag dawn-original`;`assets/` 本身就是对照快照(无需 tag 备份) |

### v11 补充决策(评审 v4 §4.2 §4.4 §4.5)

| # | 决策 | 状态 |
|---|---|---|
| 11 | 迁移 `gift_card.liquid` 模板? | ❌ **不迁移**,与 password 同处理。`templates/gift_card.liquid` 整页保留 Dawn 原版,不进 React 端 |
| 12 | 迁移 `404.json` / `customers/*.json` 等 JSON 模板? | ✅ **JSON templates 不动**,section ID 引用不变,React 端输出文件名 `react-*` 前缀仍可匹配 |
| 13 | `animations.js` 与 React 组件的共存? | ✅ **保留** `assets/animations.js`,theme.liquid 仍引用。React 组件的 `scroll-trigger` / `animate--fade-in` 等 CSS 类不变,由 `animations.js` 注入 IntersectionObserver 行为。**风险**:阶段 2 启动前需实测 React 端水合时 `animations.js` 的 DOM 操作是否触发 mismatch(可能问题:`animations.js` 在 React 水合后才执行,理论上不冲突) |
| 14 | 复杂 snippet `card-product.liquid`(651 行) + `facets.liquid`(1225 行) 单独设计评审? | ✅ **阶段 3 子阶段 3.18 / 3.20**(原 3.19 合并)前各增 1 个独立子阶段:<br>• **3.18.1**:`card-product` 设计评审(quicky-add / 体积定价 / 缺货状态 / 颜色选项)<br>• **3.18.2**:`facets` 设计评审(6 种 filter / price range / swatch / 水平 vs 垂直布局 / 排序)<br>每子阶段产出独立设计文档后再写代码 |
| 15 | `useBlockContext` / `useBlockRouter` hook | ❌ **v10 已移除**;每个 Theme Block 是独立 React entry,直接 `useBlockSettings` |
| 16 | `useStaticBlock` hook | ❌ **v10 移除**,改为 `<StaticBlock>` 组件 |
| 17 | `useShopifyAttributes` hook | ❌ **v1 移除**;插件自动注入到 block wrapper |
...
| 风险 | 影响 | 缓解 | 回退 |
|---|---|---|---|
| i18n 阶段 1.5 工期长 | 阻塞阶段 2.6+ | 阶段 1.5 与 1.1-1.4 并行,先做基础 hook | 短期保留 `useT` 走 Liquid,阶段 6 再切 |
...
| `<StaticBlock>` 组件 | 阻塞 slideshow / header | 阶段 4+ 实施;短期 workaround:用 `useRawLiquid` 注入 `{% content_for "block" %}` | - |
| 翻译键 `t: foo: bar` 误识别为 React 字符串 | UI 显示 `t:foo` | grep 所有 `t:` 引用,确认无 i18n 冲突 | - |
| 87 个 svg 转 React 组件遗漏 | 构建失败 | 阶段 2.2 写脚本自动生成,人工 spot-check 5 个 | - |
...
13. **(v11 补充)hydration 规则通过** — `pnpm hydration:check` 报告 0 error
14. **(v11 补充)单测通过** — 新增 hook/组件的 Vitest 单测覆盖 ≥ 80%,CI 必跑
15. **(v11 补充)Playwright 端到端** — 关键 section(featured-product, main-product, main-cart-items)用 Playwright 截图对比 Dawn,允许像素差 ≤ 1px
...
**v1 → v11 关键变更摘要**(完整版本历程见文档头部):
- v1:资源策略逐步删除 assets
- v2:全部内化,assets 归档
- v3:全部内化到 React,**`assets/` 永久保留**作 diff 对照
- v4:API 评审后修正 — `<ShopifyImage>` 组件 / 移除 `useShopifyAttributes` / `<BlockSlot>` 替代 useBlockLoop / bridge 不分层
- v5:统一 ID 决策(自增计数器,无外部依赖)
- v6-v9:`<ShopifyImage>` 定版(`image_url | image_tag` 管道),useForm/usePaginate 阶段 1 不实现
- v9:H3 hydration 规则降级为 warn
- v10:§6.2 范例代码与 API 签名对齐(`useSectionSettings` 解构,`useSectionIndex` 改 `useLiquidValue`)
- **v11(当前)**:阶段 1 完工标准 + 测试策略 + 复杂 snippet 设计评审子阶段
- Block 架构:Dawn 的 section block 全部升级为 Theme Block(30+ 个)
- 翻译:i18n 切换到 React 端
- 新增:image-banner.liquid 完整迁移范例(§6.2)
- 决策:§11 共 17 项已全部确认
- 验收:`git diff dawn-original -- assets/` 应为空 + §13 共 15 条
- 详细 API 设计见 `docs/2026-06-03-plugin-api-design.md`

**状态**: ✅ 评审通过(v11),可进入阶段 1 实施。详见 §10 v11 阶段 1 完工标准。
