...
> ✅ **v11 终态 + v3.0 API 更新(2026-06-08)** — 已匹配当前插件代码 API。
> **状态**: 阶段 1 核心 API 已实施,可进入阶段 2(基础组件)。
>
> **版本历程**:
...
| `useLiquidCode(code, expr?)` | `void` | 注入原始 Liquid 片段到 wrapper 外部 | ✅ DONE |
| **水合边界** | | | |
...
| **`<BlockSlot>` 编译** | — | — | **✅ DONE**:渲染为 Island 包裹的 `{% content_for 'blocks' %}` |
...
**v3.0 调整**:
- 1.1-1.3,1.8: **全部 ✅ DONE** — `useLiquid`/`useLiquidCode`/`<ShopifyImage>` 已实现
- 1.4: `BlockSlot` ✅,`StaticBlock` 🟡 TODO
- 1.5: i18n 仍待实施
- 1.6: `useImageBehavior`/`useColorScheme`/`useBlockType` 等纯 JS 工具移入用户空间 `frontend/utils/`,插件不提供专用 hook
