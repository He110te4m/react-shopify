...
> ✅ **v12 终态 + v3.1 API 更新(2026-06-08)** — 已匹配当前插件代码 API。
> **状态**: 阶段 1 runtime 核心 API 已实施,可启动阶段 2 的基础组件试迁移;i18n、hydration-rules 仍是 TODO,涉及它们的 section 不应声明完成。
>
> **版本历程**:
...
> - **v3.0 API 更新(2026-06-08)**: 匹配当前代码 — 14+ 专用 hook 简化为 `useLiquid`/`useLiquidCode` + Island/BlockSlot/ShopifyImage/ShopifyVideo 组件。纯 JS 工具函数移入用户空间。
> - **v12/v3.1 文档同步(2026-06-08)**: 清理旧 `useRawLiquid` / `useLiquidBlock` / `useLiquidValue` / `<AppBlock>` / `hydration:check` 前提,明确当前未实现项。
> - **v12.1(2026-06-08)**: `<StaticBlock>` runtime 已实现,示例见 `examples/react-shopify-example/frontend/sections/StaticBlockTest.tsx`。
>
...
> 配套文档:`docs/2026-06-03-plugin-api-design.md`(v3.1 当前实现)
...
| `image_picker` 图片 | 走 `<ShopifyImage>` 组件 | 同上(组件 + Island 水合边界,见 API 文档 §2.4) |
...
| `placeholder_svg_tag` / `inline_asset_content` / `font_face` filter | 旧计划:加入跟踪 | 当前不提供专用 hook;用 `useLiquidCode` 或 `<Island>` 传完整 Liquid 表达式,需要 bridge 时显式传 `trackedExprs` |
...
| **App Block** | 允许商家插入第三方 app 提供的 block | `{ type: "@app" }` + `<BlockSlot />`;由 Shopify 的 block 机制渲染,插件当前无 `<AppBlock>` runtime |
...
| `@app` | 多处 | **App Block** | 父 section schema 允许 `{ type: "@app" }`,通过 `<BlockSlot />` 注入 |
...
- `<StaticBlock>` 组件已实现,用于静态 block 注入位置
...
步骤 2: 资源盘点
  ...
  - 列出该 section 引用的全局 setting(animations_reveal_on_scroll 等) → `useLiquid("settings.X")`
...
步骤 4: 显示逻辑重写
  ...
  - `section.index` → `useLiquid<number>("section.index", { type: "number" })`
...
| JS | 无 | - | - |
| SVG | 无(纯文本块) | - | - |
| 全局 | `settings.animations_reveal_on_scroll` | - | `useLiquid("settings.animations_reveal_on_scroll", { type: "boolean" })` |
...
|---|---|---|
| `{%- style -%}` 媒体查询 aspect ratio | 第 3-24 行 | React `<style>` JSX 节点 + `useLiquid` 读 settings |
| `{%- style -%}` opacity ::after | 第 26-30 行 | `<style>` JSX 节点,`useLiquid` 读 `image_overlay_opacity` |
| `{%- liquid -%}` 计算 widths/sizes | 第 32-52 行 | `useImageBehavior(behavior)` 纯 JS hook(在 `frontend/utils/images.ts`) |
| 多重 `{% if %}` 链 class | 第 56 行 | 显式 `clsx()` 拼接(纯 JS,见示例) |
| `image_picker` | 第 58 行 | **`<ShopifyImage>` 组件 + Island 水合边界**(不能返回字符串) |
| `placeholder_svg_tag` | 第 87 行 | `useLiquidCode` 或 `<Island>` 包裹 |
...
export default function ImageBanner() {
  // 1. 读所有 settings — useLiquid 统一 API,返回 [value, setter] 元组
  // 路径前缀区分来源: section.settings.X / block.settings.X / settings.X
  const [image] = useLiquid<string>("section.settings.image");
  const [image2] = useLiquid<string>("section.settings.image_2");
  const [overlayOpacity] = useLiquid<string>("section.settings.image_overlay_opacity");
  ...
  const [showTextBox] = useLiquid<string>("section.settings.show_text_box");
  const [colorScheme] = useLiquid<string>("section.settings.color_scheme");
  const [stackImagesOnMobile] = useLiquid<string>("section.settings.stack_images_on_mobile");
  const [mobileContentAlignment] = useLiquid<string>("section.settings.mobile_content_alignment");
  const [showTextBelow] = useLiquid<string>("section.settings.show_text_below");

  // 2. 全局 setting + 派生 boolean
  const [animationsRaw] = useLiquid<string>("settings.animations_reveal_on_scroll");
  ...
  // useImageBehavior 是纯 JS 工具,应在 frontend/utils/images.ts 中实现
  // <ShopifyImage> 内部处理 image_tag,不需要手动传 widths/sizes
  ...
  return (
    <div id={`Banner-${sectionId}`} className={bannerClass}>
      {/* 内联样式: <style> 块替代 Dawn 的 {%- style -%} */}
      <style>{`
        .section-${sectionId}-padding {
          padding-top: ${imageHeight === 'small' ? 36 : 'var(--section-padding-top, 36px)'}px;
          padding-bottom: var(--section-padding-bottom, 36px);
        }
        .banner::after { opacity: ${overlayOpacity ? Number(overlayOpacity) / 100 : 0}; }
      `}</style>
      {isAdaptHeight && image && (
        <style>{`
...
export default function Heading() {
  // useLiquid 统一 API,路径 "block.settings.X" 区分 block 作用域
...
| `@app` | (Shopify block 机制) | 0 | schema `{ type: "@app" }` + `<BlockSlot />` |
...
### 8.1 运行时 API (v3.0,匹配当前代码)

| API | 签名 | 用途 | 状态 |
|---|---|---|---|
| **核心原语** | | | |
| `useLiquid<T>(path, opts?)` | `[T, Setter<T>]` | 统一读取 Liquid 值,替换所有专用读 hook | ✅ DONE |
| `useLiquidCode(code, trackedExprs?)` | `void` | 注入原始 Liquid 片段到 hydrate 容器之前 | ✅ DONE |
| **水合边界** | | | |
| `<Island expression={...}>` | `JSX` | 通用 hydration 边界,包裹 Liquid HTML 生成区域 | ✅ DONE |
| `<BlockSlot />` | `JSX` | block 注入点,`{% content_for 'blocks' %}` | ✅ DONE |
| **媒体组件** | | | |
| `<ShopifyImage>` | `JSX` | CDN 图片,`image_url \| image_tag` 管道 | ✅ DONE |
| `<ShopifyVideo>` | `JSX` | `video_tag` 管道 | ✅ DONE |
| **上下文** | | | |
| `LiquidDataProvider` | 组件 | bridge 数据注入到 React context | ✅ DONE |
| **静态块** | | | |
| `<StaticBlock>` | `JSX` | 静态 block 注入点 | ✅ DONE |
| **待实现** | | | |
| `useT` / `useLocale` | hook | i18n 系统(替换 Liquid `t:` filter) | 🟡 TODO |
| **用户空间(不进插件)** | | | |
| `useImageBehavior` / `useColorScheme` / `useBlockType` | hook | 纯 JS 工具,在 `frontend/utils/` 实现 | 不进插件 |
| `clsx` / `parseLiquidBoolean` | util | 同上 | 不进插件 |
| **已移除** | | | |
| `useSectionSettings` / `useBlockSettings` / `useThemeSettings` / `useLiquidValue` | — | → `useLiquid(path)` | 已移除 |
| `useRawLiquid` | — | → `useLiquidCode(code)` | 已移除 |
| `useImageUrl` / `useAsset` / `useFontFace` / `usePlaceholderSvg` | — | → `useLiquidCode` 或 `<ShopifyImage>` | 已移除 |
| `useShopifyAttributes` / `useBlockLoop` / `useBlockContext` / `useBlockRouter` | — | → 插件自动 / `<BlockSlot>` | 已移除 |
...
| `<StaticBlock>` 编译 | 不支持 | 新增 | ✅ DONE |
| i18n locale 数据生成 | 不支持 | **新增** | 同 |
| **`<BlockSlot>` 编译** | — | — | **✅ DONE**:渲染 `<shopify-block-slot data-ssg-i="__blocks__">{% content_for 'blocks' %}</shopify-block-slot>` |
| **`<ShopifyImage>` 编译** | — | — | **✅ DONE**:SSR 用 Island 包裹,不需要后处理 |
...
| 子阶段 | 新 API | PR 标题 |
|---|---|---|
| 1.1 | `useLiquid<T>(path, opts?)` + JSON bridge | ✅ DONE |
| 1.2 | `useLiquidCode(code, trackedExprs?)` | ✅ DONE |
| 1.3 | `<Island>` 水合边界 | ✅ DONE |
| 1.4 | `<ShopifyImage>` / `<ShopifyVideo>` | ✅ DONE |
| 1.5 | `<BlockSlot>` + block entry 延迟 hydrate | ✅ DONE |
| 1.6 | `<StaticBlock>` 组件 | ✅ DONE |
| 1.7 | i18n LocaleProvider + `useT` + `useLocale` + locale 生成 | 🟡 TODO |
| 1.8 | hydration-rules H2-H13 + 配置项 | 🟡 TODO |

**v3.1 调整**:
- 旧 `useRawLiquid` / `useLiquidBlock` / `useImageUrl` / `useAsset` / `useFontFace` / `usePlaceholderSvg` / `useThemeSettings` / `useSectionIndex` 不再作为插件 API 计划项
- 需要 Shopify filter 的地方用 `useLiquidCode` 或 `<Island>` 传完整 Liquid 表达式
- `useImageBehavior` / `useColorScheme` / `useBlockType` / `clsx` / `parseLiquidBoolean` 等纯 JS 工具移入用户空间 `frontend/utils/`
- `@app` 不需要 runtime `<AppBlock>`;父级 schema 允许 `{ type: "@app" }`,由 `<BlockSlot />` 交给 Shopify 渲染
...
4. **hydration 规则通过**:当前仅 H1 auto-fix 已实现;H2-H13 / `pnpm hydration:check` 是 TODO,不能作为现阶段硬门槛
...
- `pnpm test` 覆盖 hydration-fix H1;H2-H13 检查脚本待新增后再纳入 CI

**历史(v1→v3→v3.0)**:
- v1→v2: 14+ hook 设计
- v2→v3: 简化 API,移除 useBlockLoop/useShopifyAttributes
- **v3→v3.0**: 进一步简化为 2 个核心原语 + Island 水合边界,纯 JS 工具移入用户空间
...
| 15 | `useBlockContext` / `useBlockRouter` hook | ❌ **v3.0 已移除**;每个 Theme Block 是独立 React entry,直接 `useLiquid("block.settings.X")` |
| 16 | `useStaticBlock` hook | ❌ **已移除**,改为 `<StaticBlock>` 组件(已实现) |
| 17 | `useShopifyAttributes` hook | ❌ **v3.0 已移除**;插件自动注入到 block wrapper |
...
| `<StaticBlock>` 组件 | 阻塞 slideshow / header 静态私有块 | ✅ 已实现;使用 `<StaticBlock type="..." id="..." data={{...}} />` 固定注入位置 | - |
...
13. **(v12 修正)hydration 检查** — 当前仅 H1 auto-fix 有测试覆盖;H2-H13 与 `pnpm hydration:check` 脚本待实现后再作为验收项
...
- v10:§6.2 范例代码与 API 签名对齐(历史:曾把 `useSectionIndex` 改为 `useLiquidValue`)
- **v11**:阶段 1 完工标准 + 测试策略 + 复杂 snippet 设计评审子阶段
- **v12(当前)**:同步 v3.1 插件现状,明确 runtime 已完成项与 i18n/hydration-rules TODO
- **v12.1**:`<StaticBlock>` 已落地并加入 react-shopify-example 示例
