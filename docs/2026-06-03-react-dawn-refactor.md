...
| `image_picker` 图片 | 走 `<ImageTag>` 组件 | 同上(组件 + 标记替换,见 API 文档 §2.1) |
...
步骤 4: 显示逻辑重写
  ...
  - Liquid 表达式 `{{ x | y }}` → 对应 hook / 组件(`<ImageTag>` / `useT` i18n / `useSectionPadding` / ...)
...
| `image_picker` | 第 58 行 | **`<ImageTag>` 组件 + 标记替换**(不能返回字符串) |
...
import { ImageTag } from "~/components/ImageTag";         // ★ v8 组件
...
export default function ImageBanner() {
  ...
  // ★ v11 修正:useImageBehavior 不再导入(v8 ImageTag 不需要 widths/sizes)
  // 移除此 import: 见下
  ...
  return (
    <div id={`Banner-${sectionId}`} className={bannerClass}>
      ...
      {/* 图片 1 — ★ v10: ImageTag v8 签名,默认 loading="lazy" */}
      {image && (
...
| **组件(非 hook)** | | | |
| **`<ImageTag>`** | `(props: ImageTagProps) => JSX` | 输出 `image_tag` 完整 HTML 片段 | **新增**(替代 v2 的 useImageTag,避免水合 mismatch) |
...
| **移除**(v3) | | | |
| ~~`useImageTag`~~ | — | 输出 HTML 片段作为字符串 | **移除**:改为 `<ImageTag>` 组件,字符串会插入到 React 树中导致水合 mismatch |
...
| **`<ImageTag>` 编译** | — | — | **新增**:SSR 时渲染为标记,后处理替换为完整 Liquid 表达式 |
...
| 1.3 | `<ImageTag>` 组件(v8 设计:真实 `<img>` + 默认 `loading="lazy"`) | `feat(runtime): ImageTag component (v8 design)` |
...
- 1.3 改为 v8 ImageTag 设计(真实 `<img>` + 默认 `loading="lazy"`,不支持 srcSet/sizes/fetchPriority)
...
**v1 → v3 阶段 1 调整说明**:
- 1.1-1.2 不变
- 1.3 改为 `<ImageTag>` 组件(v1 错把 useImageTag 归到简单值 hook,实际是组件)
...
- v4:API 评审后修正 — `<ImageTag>` 组件 / 移除 `useShopifyAttributes` / `<BlockSlot>` 替代 useBlockLoop / bridge 不分层
- v5:统一 ID 决策(自增计数器,无外部依赖)
- v6-v8:`<ImageTag>` 重设计(默认 `loading="lazy"`),useForm/usePaginate 阶段 1 不实现
