...
import { ImageTag } from "~/components/ImageTag";         // ★ 组件 + 标记替换
...
export default function ImageBanner() {
  ...
  // 3. 派生计算
  // ★ v10 修正:useSectionIndex 暂未实现,改用 useLiquidValue("section.index", "number")
  const { value: sectionIndex } = useLiquidValue("section.index", "number");
  const isFirstSection = sectionIndex === 1;
  // ★ v10 修正:移除 fetchPriority(v8 ImageTag 不支持)
  // useImageBehavior 返回的 widths/sizes 仅在 v7+ 移除前有用,新设计中已不需要
  // 派生属性
  ...
  return (
    <div id={`Banner-${section.id}`} className={bannerClass}>
      {/* 内联样式:adapt + opacity (★ v10: 用 useSectionPadding 替代 AspectRatioStyle/OverlayOpacityStyle) */}
      <SectionPaddingStyle id={section.id} paddingTop={overlayOpacity} paddingBottom={overlayOpacity} />
      {isAdaptHeight && image && (
...
| `useStaticBlock` 编译 | 不支持 | 新增 | v10 改为 `<StaticBlock>` 组件 |
...
| `useStaticBlock` 实现复杂 | 阻塞 slideshow / header | 短期 workaround:用 `useRawLiquid` 注入 `{% content_for "block" %}`(阶段 4+ 用 `<StaticBlock>` 组件) | - |
