...
> **状态**: 阶段 1 runtime 核心 API 已实施,可启动阶段 2 的基础组件试迁移;但 `<StaticBlock>`、i18n、hydration-rules 仍是 TODO,涉及它们的 section 不应声明完成。
>
> **版本历程**:
...
| **待实现** | | | |
| `<StaticBlock>` | `JSX` | 静态 block 注入点 | 🟡 TODO |
...
| 16 | `useStaticBlock` hook | ❌ **已移除**,改为 `<StaticBlock>` 组件(待实现) |
...
| `<StaticBlock>` 组件 | 阻塞 slideshow / header 静态私有块 | 阶段 4+ 实施;短期 workaround:用 `useLiquidCode` 注入完整 `{% content_for "block" %}` 片段,但位置只能在 hydrate 容器之前 | - |
...
- **v12(当前)**:同步 v3.1 插件现状,明确 runtime 已完成项与 StaticBlock/i18n/hydration-rules TODO
