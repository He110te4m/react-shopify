...
├── snippets/                  39 native
├── templates/                 20 JSON
├── assets/                    64 CSS + 32 JS + 95 SVG  ← 全部内化到 React,源文件最终删除
...
2. **资源全部内化** — CSS、JS、SVG、icon 全部内化到 React 模块,由 Vite/esbuild 构建处理、压缩、tree-shake。`assets/` 目录作为"对照快照"保留至阶段 6 末期,之后归档或删除。
...
| 资源类型 | v1 策略 | **v2 策略(已修正)** |
|---|---|---|
| `component-*.css` / `section-*.css` / `template-*.css` | 保留 assets 短期,逐步删除 | **全部内化为同名 React 组件的 `.css` import**,由 Vite 处理;`base.css` 80KB 拆分为 `frontend/styles/tokens.css`(CSS 变量)+ 组件级 + theme.liquid 保留的 layout 段 |
| `base.css` | 拆为 tokens.css + 组件 CSS | 同上 |
| `global.js` (44KB) | 短期保留,长期重写 | **重写为 `frontend/lib/global.ts`**(pub-sub、SectionId、focus trap 等工具);原 `global.js` 在所有依赖它的 section 迁移完成后从 `theme.liquid` 引用移除并归档 |
| `cart.js` / `cart-drawer.js` / `cart-notification.js` 等 | 短期保留兜底 | **重写为 `useCart` / `useCartDrawer` / `useCartNotification` hook**;原 JS 在阶段 6 一次性从 theme.liquid 移除 |
| `product-info.js` (16.8KB) 等业务 JS | 同上 | **重写为 `useProductInfo` / `useProductForm` / `useMediaGallery` hook**;原 JS 在阶段 6 移除 |
| SVG icons (95 个) | 内化为 `.tsx` 组件 | **改为按需 import 的 `Icon` 组件**(见 §3.2),原始 svg 在 `frontend/icons/svg-source/` 留源(便于 diff),打包时由 Vite 处理为 React 组件 |
| 字体文件 (woff2) | 保留在 assets | **保留在 `assets/`,theme.liquid 引用**(字体由 Shopify font picker 配置,Dawn 的逻辑不变) |
| `image_picker` 图片 | 走 `useImageTag` | 同上 |
| 翻译 `t:` filter | 走 `useT` | **改为 React i18n**(见 §13),`locales/*.json` 由 Vite 插件转换为 `frontend/i18n/{locale}.json`,运行时按当前 locale 加载 |
...
> **v1 → v2 关键变化**:不再"短期保留原 assets,逐步删除",改为"全部重写为 React/TS,React 接管后原 assets 一次性归档"。`assets/` 目录在阶段 0-2 维持不动,React 端完全从零编写,阶段 6 末期对照通过后从 `theme.liquid` 移除 `<script>` / `<link>` 引用并归档原始 `assets/` 子集。
...
**theme.liquid 净删除清单**:
- ❌ `constants.js` / `pubsub.js` / `global.js` / `details-disclosure.js` / `details-modal.js` / `search-form.js` — 重写到 `frontend/lib/`
- ❌ `cart-drawer.js` / `cart-notification.js` / `predictive-search.js` / `localization-form.js` — 重写到 React hook
- ❌ `component-cart-drawer.css` / `component-cart-items.css` / `component-cart.css` / `component-discounts.css` / `component-totals.css` / `component-predictive-search.css` / `component-localization-form.css` / `component-price.css`(使用) — 内化到 React 组件
...
- 5.4 从 `theme.liquid` 移除 `global.js` / `cart.js` / `pubsub.js` 等 `<script>` 引用(已重写到 `frontend/lib/`)
- 5.5 从 `theme.liquid` 移除 `component-*.css` 等 `<link>` 引用(已内化到 React)

### 阶段 6 — 归档 + 文档

- 6.1 `git tag dawn-original`(保留原 assets 作为对照快照)
- 6.2 评估:若全部 React 化且无 hydration 回归,**不删除** `assets/`(用户确认 v2 决策)
- 6.3 写 `frontend/README.md` + `QUIRKS.md`
- 6.4 Lighthouse 比对(目标 ≥ Dawn 基线)
...
| 5 | 删除 25 个彩蛋 svg(apple/banana/...) | ❌ 不删除,内化到 `<Icon name="apple" />` |
| 6 | 拆分未引用 CSS/JS 文件? | ❌ 不拆,保留 `assets/` 整体作为对照快照 |
| 7 | 迁移 `meta-tags` snippet? | ❌ 不迁移,SEO 关键 |
| 8 | 翻译方案 | ✅ **i18n 走 React,不走 Liquid t filter**,初始语言从 `request.locale.iso_code` 注入(见 §13) |
| 9 | 删除 `assets/*.js` 旧文件? | ❌ 不删,作为对照快照保留,`theme.liquid` 引用逐步移除但源文件归档 |
| 10 | 备份原 Dawn 资源? | ✅ `git tag dawn-original` 即可 |
...
10. **assets 引用计数** — React 化后,`theme.liquid` 中 `<link href="component-*.css">` / `<script src="*.js">` 数量为 0(仅保留 animations.js / base-reset.css / importmap / 字体)
