    ...
    <div id={`Banner-${section.id}`} className={bannerClass}>
      ...
      {image && (
        <div className="banner__media media">
          <ImageTag image={image} width={3840} widths={widths} sizes={sizes}
      ...
      {image2 && (
        <div className="banner__media media banner__media-half">
          <ImageTag image={image2} width={3840} widths={widths} sizes={halfWidth} fetchPriority={fetchPriority} />
        </div>
      )}
...
| 新 hook | 签名 | 用途 | v3 调整 |
|---|---|---|---|
| `useRawLiquid` | `(code: string) => string` | 关键:插入任意 Liquid 字符串到当前渲染位置 | 不变(标记替换模式) |
| `useForm` | `(formType, options?) => { FormOpen, FormClose, formState, submit }` | 包装 `{% form %}` | 不变 |
| `usePaginate` | `(items, perPage, param?) => { currentItems, page, totalPages, PaginateOpen, PaginateClose, PaginationEl }` | 包装 `{% paginate %}` | 不变 |
...
| `useStaticBlock` | `(spec: { type, id, data? }) => ReactNode` | 静态 block 位置 | 新增 |
| **组件(非 hook)** | | | |
...
| **`<StaticBlock>`** | `(props: { type, id, data? }) => JSX` | 渲染 `{% content_for "block", ... %}`,指定 block 位置 | **新增**(替代 v2 的 useStaticBlock) |
| **移除**(v3) | | | |
...
| `useStaticBlock` 编译 | 不支持 | 新增 | 改为 `<StaticBlock>` 组件 |
...
| 1.3 | `<ImageTag>` 组件 + 标记替换机制 | `feat(runtime): ImageTag component with marker substitution` |
...
| 1.6 | `useSectionPadding`, `useColorScheme`, `useThemeSettings`, `useImageBehavior`, `useSectionAspect`, `useBlockType` | `feat(runtime): style and theme hooks` |
