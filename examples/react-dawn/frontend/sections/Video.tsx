...
export default function Video() {
  ...
  useLiquidCode(`{{ 'video-section.css' | asset_url | stylesheet_tag }}`, []);
...
{%- capture sizes -%}
  ...
  {%- endif %}
{%- endcapture -%}`, ["section.settings.video", "section.settings.video_url", "section.settings.description", "section.settings.cover_image", "section.settings.full_width"]);
      ...
      >
        {heading && (
          <div className={fullWidth ? "page-width" : undefined}>
            <div
              className={clsx("title-wrapper", "title-wrapper--no-top-margin", {
        ...
        <div dangerouslySetInnerHTML={{ __html: "<!-- deferred-media rendered by Liquid -->" }} />
      </div>
    </div>
  );
}
